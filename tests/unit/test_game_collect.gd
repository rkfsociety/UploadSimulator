extends RefCounted
## Сбор денег: сейф сети → касса через коллектор.
...
func run() -> Array[String]:
	...
	data.set_network_balance(100.0)
	data.set_money(50.0)
	var bonus := GameBonus.effect_at_level("collector", 1)
	var payout := 100.0 * (1.0 + bonus)
	data.set_network_balance(0.0)
	data.add_money(payout)
	if not is_equal_approx(data.get_network_balance(), 0.0):
		errors.append("после сбора сейф сети должен быть 0")
