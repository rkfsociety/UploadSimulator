...
func run() -> Array[String]:
	...
	if not wiring.try_connect_ports(d_uid, "file_out", s_uid, "file_in"):
	...
	if wiring.try_connect_ports(s_uid, "file_out", u_uid, "file_in"):
...
func _place_modules(field: GameFieldService, data: GameStateData) -> Dictionary:
	...
	for type_id in ["downloader", "storage", "uploader"]:
		...
		uids[type_id] = field.place_block(type_id, gx, 0)
