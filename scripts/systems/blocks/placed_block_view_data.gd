...
static func from_instance(uid: String, type_id: String) -> PlacedBlockViewData:
	...
	data.metric = GameState.get_block_metric(uid)
	var disp: Dictionary = GameState.get_block_display(uid)
	...
	var lvl: int = GameState.get_instance_level(uid)
	var cost: int = GameState.get_instance_upgrade_cost(uid)
	data.upgrade_text = "↑ Ур.%d · $%d" % [lvl + 1, cost]
	data.upgrade_enabled = GameState.can_upgrade_instance(uid)
	return data
