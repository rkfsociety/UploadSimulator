...
func setup(uid: String, world_pos: Vector2, gem_amount: int) -> void:
	...
	custom_minimum_size = Vector2(FieldMapConstants.DIAMOND_PICKUP_SIZE, FieldMapConstants.DIAMOND_PICKUP_SIZE)
...
func _draw() -> void:
	...
	if amount > 1:
		...
		draw_string(font, center + Vector2(-tw * 0.5, 4.0), label, HORIZONTAL_ALIGNMENT_LEFT, -1, fs, Color.WHITE)
